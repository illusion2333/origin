...
net:
  ...
  # 种子节点地址列表，用于节点发现，可选项
  # 网络模块在链初始化阶段会主动读取链配置（例如:bc1.xml）中共识节点列表，并将共识节点地址添加为种子节点
  seeds: 
...
2.假设节点N的TLS证书是由组织ID为“wx-org2.chainmaker.org”的CA证书签发（节点的TLS证书配置在chainmaker.yml中的net.tls.priv_key_file和net.tls.cert_file，分别对应的节点TLS私钥和证书文件，具体详见上方配置说明）
...
![与其他模块交互](../images/P2P网络-与其他模块交互图.jpg)
...
- 第六段：节点NodeId，与TLS证书配套，根据TLS证书通过特定算法计算而来（根据TLS证书获得相应NodeId请参考[CMC工具](../dev/3、命令行工具.md)中证书管理功能）
