...
拉取镜像
```sh
docker pull huzhenyuan/chainmaker-cpp-contract:1.1.0
```

请指定你本机的工作目录$WORK_DIR，例如C:\tmp，挂载到docker容器中以方便后续进行必要的一些文件拷贝

```sh
docker run -it --name chainmaker-cpp-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-cpp-contract:1.1.0 bash
# 或者先后台启动
docker run -d --name chainmaker-cpp-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-cpp-contract:1.1.0 bash -c "while true; do echo hello world; sleep 5;done"
# 再进入容器
...
拉取镜像
```
docker pull huzhenyuan/chainmaker-go-contract:1.1.0
```

请指定你本机的工作目录$WORK_DIR，例如C:\tmp，挂载到docker容器中以方便后续进行必要的一些文件拷贝

```sh
docker run -it --name chainmaker-go-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-go-contract:1.1.0 bash
# 或者先后台启动
docker run -d  --name chainmaker-go-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-go-contract:1.1.0 bash -c "while true; do echo hello world; sleep 5;done"
# 再进入容器
...
拉取镜像
```sh
docker pull huzhenyuan/chainmaker-rust-contract:1.1.0
```

请指定你本机的工作目录$WORK_DIR，例如C:\tmp，挂载到docker容器中以方便后续进行必要的一些文件拷贝

```sh
docker run -it --name chainmaker-rust-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-rust-contract:1.1.0 bash
# 或者先后台启动
docker run -d  --name chainmaker-rust-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-rust-contract:1.1.0 bash -c "while true; do echo hello world; sleep 5;done"
# 再进入容器
...
pub extern "C" fn save() {
    ...
    //发送合约事件
    //向topic:"topic_vx"发送2个event数据，file_hash,name
    let mut arr: Vec<String> = Vec::new();
    arr.push(file_hash.to_string());
    arr.push(file_name.to_string());
...
拉取镜像
```sh
docker pull huzhenyuan/chainmaker-solidity-contract:1.1.0
```

请指定你本机的工作目录$WORK_DIR，例如C:\tmp，挂载到docker容器中以方便后续进行必要的一些文件拷贝

```sh
docker run -it --name chainmaker-solidity-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-solidity-contract:1.1.0 bash
# 或者先后台启动
docker run -d --name chainmaker-solidity-contract -v $WORK_DIR:/home huzhenyuan/chainmaker-solidity-contract:1.1.0 bash -c "while true; do echo hello world; sleep 5;done"
# 再进入容器
