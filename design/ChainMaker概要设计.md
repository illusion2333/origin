...
- 区块链（BlockChain）：使用密码技术链接，将共识确认过的区块按顺序追加而形成的分布式账本。

注：区块链旨在防止篡改，并创建最终的、确定的和不变的账本记录。

（来源：ISO 22739:2020，3.11）

- 共识（Consensus）：分布式账本间达成的协议，即交易已验证并且分布式账本包含已验证交易的一致性集合和顺序。

注：共识不等同于所有节点同意。不同的共识算法细节有所不同，这是一种设计与另一种设计之间的区别特征。

（来源：ISO 22739:2020，3.39）

- 智能合约（SmartContract ）：一个智能合约是一套以数字形式定义的承诺（commitment），包括合约参与方可以在上面执行这些承诺的协议。

（来源：尼克·萨博（Nick Szabo）1995）

- 节点（Peer）：维护分布式账本的网络节点，包括共识节点、全节点、轻节点。

- 链（Chain）：由多个节点组成的区块链网络，在这个网络下采用相同的共识算法、加密算法进行记账，有统一的管理机制、交易规则，以及统一的身份标识服务。

- 组织/机构（Organization）：节点的管理者，可以是企业或部门等相对独立的，具有自主决策权的单位。一个组织可以管理多个节点。

- 跨合约调用：合约本身可以看做是一个“智能机器人“，当合约A调用合约B时，就发生了跨合约调动。跨合约调用时，合约执行环境可以是A的环境，也可以是B的环境。

- 多链：ChainMaker可以并行创建多条链来运行，链与链之间共享网络通讯、身份权限等服务，但账本、共识过程彼此独立。通过多链特性可以满足业务大容量、数据隔离等需求。
...
监控&运维模块包括监控和运维两个子模块，其中监控分为：

（1）系统级监控：监控服务器资源，包括：CPU、内存、磁盘、网络IO等。

（2）应用级监控：监控区块链平台chainmaker的关键指标，如：进程的内存使用、进程CPU使用、进程文件句柄数情况、Txpool大小、出块时间、共识状态、智能合约引擎执行情况、交易接收和处理吞吐量等。

（3）业务级监控：监控业务定制的指标，如：接口X的健康度、交易的次数/金额、

运维模块提供系统的部署、版本的升级维护等功能，具体包括采用Docker方式进行chainmaker的快速部署，对chainmaker可执行程序的版本升级，版本之间的兼容性管理，以及各模块版本之间的配套关系。

在运维平台领域做的比较完善的是蓝鲸平台（https://bk.tencent.com），也可采用Grafana+prometheus或Zabbix自己搭建。
