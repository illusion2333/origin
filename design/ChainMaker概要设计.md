...
| ----------------- | ---------------------------------- | ---------------------------- | ------------------------------------------------------------ |
| 整体架构          |                                    |                              | 明超                                                         |
| 核心引擎          | 实现各子模块管理和调度             | 实现各子模块管理和调度       | 区块提议\*、校验\*(振远、殷舒)                               |
| 节点组网          | libp2p（4节点）  gRPC              | libp2p（4/7/10节点）         | libp2p国密集成（瑞波）                                       |
| 身份权限管理      | 公私钥+白名单                      | 公私钥和证书+白名单          | 公私钥对接其他模块\*（张韬、瑞波、振远、殷舒），证书开发     |
| 共识算法          | TBFT                               | TBFT                         | 区块广播\*（智超），共识投票\*（#1）                         |
| 区块同步          |                                    |                              | 方案及开发\*（李欣），独立模块                               |
| 智能合约          | WASM通用合约（go和C++）            | WASM通用合约（go和C++）      | 10月通用合约（读+写）（振远、石鑫）                          |
| 交易调度          | 并行                               | 无冲突并行DAG                | DAG生成\*（振远，瑞波）                                      |
| TxPool            | 校验和防重                         | 队列持久化，校验和防重       | 交易并行校验\*（殷舒），队列持久化                           |
| 跨链              | --                                 | --                           |                                                              |
| 隐私保护          | --                                 | --                           |                                                              |
| 账本存储          | levelDB                            | levelDB/rocksDB              | columnfamily定义\*，key定义\*，所有接口实现\*，levelDB分表（长辉） |
| 链启动            |                                    |                              | 链初始化\*，启动\*，创世块\*（明超、#2、#3）                 |
| 配置链            |                                    |                              | 配置合约\*，配置加载\*（#4）                                 |
| 装配线            | --                                 | --                           | go的依赖注入                                                 |
| RPC接口、CLI及SDK | RPC服务端接口、CLI及SDK（go+java） | --                           | RPC接口定义\*、CLI\*和SDK封装\*（Kevin、Jason）              |
| 密码算法          | go版本国密、ECDSA（确定曲线）      |                              | 优化，与其他模块对接，gRPC国密\*（Jason）                    |
| 管理平台          |                                    | 基础配置，区块浏览，运行情况 | dashboard、区块浏览【前端】                                  |
| 监控&运维         |                                    | 资源和系统监控               | 基于docker、K8s的资源监控【docker、K8s】                     |
| 测试工具          |                                    | 测试模式下mock模拟共识异常   | 测试用例                                                     |
